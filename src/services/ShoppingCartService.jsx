...
export const deleteShoppingCartById = async (shoppingCartID) => {
  try {
    const response = await handleShoppingCartApi.deleteShoppingCartById(
      shoppingCartID
    );
    if (response && response.data) {
      return response.data;
    ...
    if (error.response) {
      ...
      if (error.response.status === 404) {
        ...
        return { error: "Bạn không có quyền xóa giỏ hàng này!" };
      } else if (error.response.status === 401) {
        return { error: "Vui lòng đăng nhập để thực hiện thao tác này!" };
      } else if (error.response.data && error.response.data.message) {
        return { error: error.response.data.message };
